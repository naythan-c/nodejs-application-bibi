...
exports.run = async (client, message, args, func) => {
  if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send("You can't use this! Permission required: `Manage Guild`");
      ...
      let welcomeMessage;
      if(!args[1]){
          return message.channel.send("Please input a welcome message!")
      } else {
      ...
      if(dat===null){
        await collection.insertOne({"guildId":message.guild.id,"welcomeMessage":welcomeMessage})
      }
      else{
     ...
     .setTitle("Successfully set the welcome message to:")
